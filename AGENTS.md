...
The proxy uses **TLS inspection (ssl_bump)**: all HTTPS connections are intercepted and re-encrypted using a per-install CA certificate that this container trusts. Full request URLs and response codes are visible in proxy logs. This is transparent to you — all HTTPS connections work normally.

...
# ---------------------------------------------------------------------------
# The proxy requires config/ca.crt and config/ca.key to start (ssl_bump).
# These are generated by install.sh. For tests run from the repo directly
# (without install.sh having been run), generate a throwaway CA cert.
# ---------------------------------------------------------------------------
...
# The entrypoint installs the CA cert into the system trust store via
# update-ca-certificates. After the entrypoint runs, the cert should appear
# at /etc/ssl/certs/sandboxed-copilot-ca.pem.
# run_online starts both the copilot container and the proxy — the CA cert
# bind-mount (/etc/sandboxed-copilot-ca.crt) is present in this mode.

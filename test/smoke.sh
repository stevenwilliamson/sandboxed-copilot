...
# Also regenerate if an existing cert has expired (e.g. stale from a previous
# interrupted test run that generated a 1-day cert).
# ---------------------------------------------------------------------------
_test_generated_ca=false
_need_ca=false
if [ ! -f "${PROJECT_DIR}/config/ca.crt" ] || [ ! -f "${PROJECT_DIR}/config/ca.key" ]; then
    _need_ca=true
elif ! openssl x509 -in "${PROJECT_DIR}/config/ca.crt" -checkend 3600 -noout 2>/dev/null; then
    echo "── Removing expired CA cert (will regenerate)..."
    rm -f "${PROJECT_DIR}/config/ca.crt" "${PROJECT_DIR}/config/ca.key"
    _need_ca=true
fi
if [ "$_need_ca" = true ]; then
...
EXFIL_ENTRY=$(docker exec "$PROXY_CID" \
    sh -c "grep -c '${EXFIL_TEST_DOMAIN}' /var/log/squid/exfil.log 2>/dev/null; true")

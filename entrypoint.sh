...
# The CA cert is generated by the proxy container on first run and stored in
# the ca-certs named volume, mounted read-only at /etc/squid/ca.
# Without this step, all HTTPS connections through the proxy would fail with
# a certificate verification error.
# Skipped gracefully if the cert is absent (e.g., proxy not yet started).
# ---------------------------------------------------------------------------

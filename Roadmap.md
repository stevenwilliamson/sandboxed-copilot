...
- [x] **E2: Block repo write paths on existing repos** *(score 4)* — `PUT /repos/*/contents/*` (file write), `PATCH /gists/{id}` (edit existing gist), `POST /repos/*/git/blobs`, `POST /repos/*/git/trees`, `POST /repos/*/git/commits`, `PATCH /repos/*/git/refs/*`; added to `blockedGitHubAPIEndpoints`. Note: `POST /repos/*/issues` and `POST /repos/*/issues/comments` are excluded to preserve Copilot's issue-creation and PR-comment workflows.
...
- [x] **T2-G: Multi-variant images** — three built-in image variants (`minimal` / `standard` / `full`) with a multi-stage Dockerfile; `full` includes Google Chrome for headless browser automation; `--image` flag and `.sandboxed-copilot [image]` section for custom images and Dockerfiles; `sandboxed-copilot build [variant]` subcommand

...
The CA cert and key are stored in the `ca-certs` Docker named volume. The proxy container generates them on first run (RSA 4096, 5-year validity). This is fully self-bootstrapping — no pre-flight `install.sh` step is required.

| File | Location | Accessible from |
|------|----------|----------------|
| `ca.key` — private key (chmod 600) | `ca-certs` volume at `/etc/squid/ca/ca.key` | proxy container only (volume mounted writable) |
| `ca.crt` — public cert | `ca-certs` volume at `/etc/squid/ca/ca.crt` | proxy container (writable) + copilot container (`:ro`, installed into trust store at startup) |

- Generated once per Docker host (volume persists across container restarts and image upgrades)
- Regenerated automatically if the `ca-certs` volume is deleted
- **Never added to the host system trust store** — only the copilot container trusts it
- Removed by `uninstall.sh` (deletes the `ca-certs` volume)
